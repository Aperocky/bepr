...
use crate::{
    client,
    server::{self, ServerConfig, DEFAULT_SERVER_CONFIG},
};
...
fn usage() -> String {
    "usage: bepr server [--config server.conf]\n       bepr client [--config client.conf]\n       bepr connect <client_id> [--config server.conf]\n       bepr keygen".to_string()
}

async fn connect(args: Vec<String>) -> Result<(), String> {
    let (client_id, config_path) = parse_connect_args(args)?;
    let config = ServerConfig::from_file(&config_path).map_err(|err| err.to_string())?;
    let mut stream = UnixStream::connect(&config.operator_socket)
        .await
...
fn parse_connect_args(args: Vec<String>) -> Result<(String, String), String> {
    match args.as_slice() {
        [client_id] => Ok((client_id.clone(), DEFAULT_SERVER_CONFIG.to_string())),
        [client_id, flag, path] if flag == "--config" || flag == "-c" => {
            Ok((client_id.clone(), path.clone()))
        }
        _ => Err("usage: bepr connect <client_id> [--config server.conf]".to_string()),
    }
}
